...
public class ABGaugeView: UIView {
    ...
    // MARK:- @IBInspectable
    @IBInspectable public var colorCodes: String = "ffffff,ffffff,ffffff,ffffff,ffffff"
    ...
    fileprivate var previousNeedleValue = CGFloat(0)
    @IBInspectable public var needleValue: CGFloat = 0 {
        didSet {
            setNeedsDisplay()
            previousNeedleValue = oldValue
        }
    }
    ...
    func createArcWith(startAngle: CGFloat, endAngle: CGFloat, arcCap: CGLineCap, strokeColor: UIColor, center:CGPoint) {
        ...
        let lineWidth: CGFloat = self.frame.width/12
        // 2
    ...
    func drawNeedle() {
        ...
        let previousNeedleValue = radian(for: self.previousNeedleValue) + firstAngle
        animate(triangleLayer: triangleLayer, shadowLayer: shadowLayer, fromValue: previousNeedleValue, toValue: needleValue, duration: 0.5) {
        }
    }
